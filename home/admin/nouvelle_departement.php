...
if (isset($_SESSION['nom'],$_SESSION['prenom'],$_SESSION['mail'],$_SESSION['reference'],$_GET['departement'])) {
    if($_SESSION['acces'] == 'admin'){
        ...
        if ($ligne == 1) {
            ...
            $donnees_departement = $recuperation_departement->fetch();

            $nom = $donnees_departement['nom'];
            $rensponsable = $donnees_departement['rensponsable'];
            $reference = $donnees_departement['reference'];
        ...
        if (isset($_POST['bouton_envoi'])) {
            if (isset($_POST['nom'],$_POST['rensponsable']) AND !empty($_POST['nom'])) {
                ...
                $modification_departement = $db->prepare('UPDATE `departement` SET `nom`= ?, `rensponsable`= ? WHERE (reference = ?)');
                $modification_departement->execute(array($nom,$rensponsable,$reference));
                
                $nbr_ligne = $modification_departement->rowCount();
