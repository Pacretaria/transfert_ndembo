...
if (isset($_SESSION['nom'],$_SESSION['prenom'],$_SESSION['mail'],$_SESSION['reference'])) {
    ...
    if(isset($_GET['ref'])){
        ...
        if(isset($_POST['bouton_envoi'])){
            ...
            if (isset($_FILES['fichier'])) {
                ...
                if($check ==! false){
                    ...
                    if (move_uploaded_file($_FILES['fichier']['tmp_name'], $uploadirFile)) {
                        ...
                        $mise_jour = $db->prepare('UPDATE `utilisateurs` SET `profil`= ?,`valeur`= ? WHERE (reference = ?)');
                        $mise_jour->execute(array($_FILES['fichier']['name'],2,$reference));
            ...
            <body>
                <div id="message">
                    <h1> Televerser un nouveau Profil </h1>
                    <form action="?ref=<?php echo $reference?>" enctype="multipart/form-data" method="post">
                        <div class="portion">
                            <div>
                                <label for="fichier"> Profil </label>
                                <input type="file" name="fichier" accept="image/" id="nom">
                            </div>
