 ...
 if($_SESSION['acces'] == 'Analyste'){
    ...
    if (isset($_GET['joueur'])) {
        ...
        if (isset($_POST['bouton_envoi'])) {
            ...
            if (isset($_POST['competition'],$_POST['nbr_matches'],$_POST['heure_jouer'],$_POST['passes_decisives'],$_POST['but'],$_POST['passes_reussis'],$_POST['recuperation_balles'],$_POST['note'])) {
                ...
                $enregistrement_joueur = $db->prepare('INSERT INTO `performances`(`joueurs`, `competions`, `nbr_matches`, `nbr_minute_jouer`, `nbr_passes_decisives`, `nbr_buts`, `nbr_passes_reussis`, `nbr_recuperation_balles`, `notes`, `date`, `references`)  VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                $enregistrement_joueur->execute(array($donnees_joueur['reference'],$competition,$nbr_matches,$minute_jouer,$passes_decisives,$but,$passes_reussis,$recuperation_balles,$note,date('d/m/Y'),$reference));
        ...
        <form action="" method="post" id="form_enregistrement">
            ...
            <div id="conteneur">
                <div class="portion">
                    ...
                    <div>
                        <label for="manager">Compétition</label>
                        <select name="competition" id="">
                            <?php while($donnees_competitions = $recuperation_competitions->fetch()){ ?>
                            <option value="<?php echo $donnees_competitions['reference'] ?>"> <?php echo $donnees_competitions['nom'] ?> </option>
                            <?php
                            }
                            ?>
                        </select>
                    ...
                    <div>
                        <label for="nbr_matches">Matches</label>
                        <input type="number" name="nbr_matches" value="1" id="nbr_matches">
                    </div>
