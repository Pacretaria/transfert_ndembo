...
if (isset($_SESSION['nom'],$_SESSION['prenom'],$_SESSION['mail'],$_SESSION['reference'])) {
    if($_SESSION['acces'] == 'Analyste'){
        ...
        if (isset($_POST['recherche'])) {
            ...
            $recuperation = $db->prepare('SELECT * FROM `jouers` WHERE `nom` LIKE :recherche OR `position` LIKE :recherche OR `club` LIKE :recherche OR `pied` LIKE :recherche OR `manager` LIKE :recherche OR `nationalite` LIKE :recherche ORDER BY prix DESC');
        ...
        <table id="resultat_recherche">
            ...
            <tbody>
                <?php
                    ...
                    ?>
                    <tr>
                        <td><a href="enregistrement.php?joueur=<?php echo $donnees_joueurs['reference'] ?>"><img src="../media/<?php echo $donnees_joueurs['profil'] ?>" alt="profil" class="profil_joueurs"> <p> <?php echo $donnees_joueurs['nom'].' '.$donnees_joueurs['prenom'] ?> <br><span> <?php echo $donnees_joueurs['position'] ?> </span> </p></a></td>
